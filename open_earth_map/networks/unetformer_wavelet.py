...
#     def forward(self, x, LL, LH, HL, HH ):
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.pre_conv2 = ConvBNReLU(decode_channels, encoder_channels[-1], kernel_size=3)
        self.post_conv = Conv(encoder_channels[-1], decode_channels, kernel_size=1)
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        LL,LH,HL,HH = res4
        x = self.b4(self.pre_conv(LL))
        x = self.pre_conv2(x)
        x = self.upsample(x, LH, HL, HH)
        x = self.post_conv(x)

        LL,LH,HL,HH = res3       
        x = self.p3(x, LL, LH, HL, HH)
        x = self.b3(x)

        LL,LH,HL,HH = res2
        x = self.p2(x, LL, LH, HL, HH)
        x = self.b2(x)

        LL,LH,HL,HH = res1
        x = self.p1(x, LL, LH, HL, HH)
...
class UNetFormer_Modified(nn.Module):
    def __init__(
        self,
        decode_channels=64,
        dropout=0.1,
...
class Encoder(nn.Module):
    def __init__(
        ...
        dim = encode_channels
        self.dim = encode_channels
        self.ConvBlock1 = block(3, dim, strides=1)
        self.pool1 = Downsample(wavename = wavename)

        self.ConvBlock2 = block(dim, dim*2, strides=1)
        self.pool2 = Downsample(wavename = wavename)
       
        self.ConvBlock3 = block(dim*2, dim*4, strides=1)
        self.pool3 = Downsample(wavename = wavename)
        
        self.ConvBlock4 = block(dim*4, dim*8, strides=1)
        self.pool4 = Downsample(wavename = wavename)
